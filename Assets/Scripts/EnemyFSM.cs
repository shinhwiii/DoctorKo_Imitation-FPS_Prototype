...
public class EnemyFSM : MonoBehaviour
{
    ...
    [SerializeField]
    private float pursuitLimitRange = 10;                       // 추적 범위 (이 범위 밖으로 나가면 "Wander" 상태로 변경
    ...
    [SerializeField]
    private float attackRange = 5;                              // 공격 범위 (이 안에 들어오면 "Attack" 상태로 변경
    [SerializeField]
    ...
    {
        // 현재 재생 중인 상태와 바꾸려고 하는 상태가 같으면 바꿀 필요가 없기 때문에 return
    ...
    private IEnumerator Wander()
    {
        ...
        while (true)
        {
            ...
            // 목표 위치와 근접하게 도달하거나 너무 오랜 시간 동안 "배회" 상태에 머물러 있으면
    ...
    private Vector3 CalculateWanderPosition()
    {
        ...
        // 자신의 위치를 중심으로 반지름(wanderRadius) 거리, 선택된 각도(wanderJitter)에 위치한 좌표를 목표 지점으로 연결
